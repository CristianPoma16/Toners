...
var menu = {
  ...
  elements: [
    ...
    { id: 'btnCerrarSesion', view: 'button', icon: 'mdi mdi-table', label: 'Cerrar Sesion', autowidth: true, click: close }
  ]
}
...
function save_prodcuts() {
  if ($$('datos_pro').isVisible()) {
    const save = $$("id_form_prod").getValues();
    if ($$("id_form_prod").validate()) {
    ...
    if ($$('datos_cli').isVisible()) {
      const save = $$("id_form_cli").getValues();
      if ($$("id_form_cli").validate()) {
...
function del() {
  if ($$('datos_pro').isVisible()) {
    ...
    webix.confirm("Eliminar producto seleccionado?", "confirm-warning").then(function () {
      ...
      var var_delete = {
        id: $$("datos_pro").getSelectedItem().id
      }
...
function close() {
  ...
  webix.confirm("Actualizar producto seleccionado?", "confirm-warning").then(function () {
    ipcRenderer.send('closeWin', 'pong')
  })
} 
...
function show_products() {
  return webix.ui({
    ...
    body: {
      ...
      template: "#cod_pro# #nom_pro# #mar_pro#"
    }
  })
}
function openForm() {
